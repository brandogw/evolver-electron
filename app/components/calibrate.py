...
def sigmoid_fit(calibration, fit_name, params, graph = False):
    coefficients = []
...
def linear_fit(calibration, fit_name, params, graph = False):
    coefficients = []
...
def constant_fit(calibration, fit_name, params, graph = False):
...
def three_dimension_fit(calibration, fit_name, params, graph = False):
...
if __name__ == '__main__':
    ...
    parser.add_option('-s', '--show-graphs', action = 'store_true', dest = 'graph', help = "Show graphs for fits")
    ...
    if cal_name is not None and not get_names:
        if fit_type == "sigmoid":
            fit = sigmoid_fit(calibration, fit_name, params, graph)
